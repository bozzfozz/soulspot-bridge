...
async def example_cache_usage():
    ...
    track = {"id": "track-456", "name": "Test Track", "artists": [{"name": "Test Artist"}]}
    ...
    from soulspot.domain.value_objects import TrackId, FilePath
...
async def example_job_queue():
    ...
    from soulspot.application.workers import JobQueue, JobType, JobStatus
...
async def example_token_manager():
    ...
    class MockSpotifyClient:
        async def get_authorization_url(self, state, code_verifier):
